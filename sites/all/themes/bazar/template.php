...
//            $bazarClassList['reset-bootstrap']['external']=false;
            ...
            $bazarJsConfigButtom = 'var thickboxL10n = {"next":"Next >","prev":"< Prev","image":"Image","of":"of","close":"Close","noiframes":"This feature requires inline frames. You have iframes disabled or your browser does not support them.","loadingAnimation":"http:\/\/demo.yithemes.com\/bazar\/wp-includes\/js\/thickbox\/loadingAnimation.gif","closeImage":"http:\/\/demo.yithemes.com\/bazar\/wp-includes\/js\/thickbox\/tb-close.png"};'."\n"
                . 'var l10n_handler = {"map_close":"[x] Close","map_open":"[x] Open"};'."\n"
                . 'var yith_woocompare = {"nonceadd":"2ff540915e","nonceremove":"e0a22b5f3d","ajaxurl":"http:\/\/demo.yithemes.com\/bazar\/wp-admin\/admin-ajax.php","actionadd":"yith-woocompare-add-product","actionremove":"yith-woocompare-remove-product","actionview":"yith-woocompare-view-table","added_label":"Added","table_title":"Product Comparison","auto_open":"yes"};'."\n"
                . 'var yith_wcwl_l10n = {"out_of_stock":"Cannot add to the cart as product is Out of Stock!"};'."\n"
                . 'var yith_wcan = {"container":".products","pagination":".general-pagination","result_count":".woocommerce-result-count"};'."\n"
...
function bazar_preprocess_region(&$vars, $hook){
    ...
    //$default_product = commerce_product_load($default_product_ref['product_id']);
    //$price = field_view_field('commerce_product', $default_product, 'commerce_price')[0]['#markup'];
    $vars['node-instance'] = $vars['elements']['system_main']['nodes'];
    if(!empty($vars['elements']['system_main']['nodes'])){
         // $vars['elements']['#nodes'] = $vars['elements']['system_main']['nodes'];
    }

    //set defaults for shopping cart

        //var_dump('process_region');


    //$nodes = $vars['elements']['system_main']['nodes'];
...
function bazar_breadcrumb($variables) {
    ...
    if ((arg(0) == 'node') && is_numeric(arg(1)) && ($node = node_load(arg(1)))) {
        ...
        switch($node->type){
            case 'template_display':
                $breadcrumb[] = l(t('Design Templates'), 'drupal-templates-for-sale');
