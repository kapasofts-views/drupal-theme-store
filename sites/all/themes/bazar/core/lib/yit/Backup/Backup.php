	    ...
	    foreach( self::$wptables as $table ) {                               
	        ...
	        foreach( $db[$table] as $id => $data ) {                         
	            if( $wpdb->insert( $wpdb->$table, $data ) ) {
                    ...
                    echo '<p style="background-color: #f00">#### Unable to insert data in the table `' . $wpdb->$table . '`</p>';
                    yit_debug( $data );
                }   
	    ...
	    if ( ! empty( $tables ) )
	        foreach ( $tables as $table ) {    
                ...
                if( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}{$table}'" ) == $wpdb->prefix . $table ) {
    	            ...
    	            #if( $insert )
    	            #    yiw_string_( '', sprintf( __( 'Insert new values into %s table', 'yit' ), $wpdb->prefix . $table ), '...</p>' );
    	            #else
    	            #    yiw_string_( '', sprintf( __( 'Error during insert new values (IDs: %s), in %s table', 'yit' ), implode( $error_data, ' ' ), $wpdb->prefix . $table ), '...</p>' );
                }
	        }
